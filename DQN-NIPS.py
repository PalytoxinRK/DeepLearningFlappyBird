...
OBSERVE = 100. # 经验池的样本数
EXPLORE = 120000. # frames over which to anneal epsilon
FINAL_EPSILON = 0.001 # 结束探索时候的选择动作的ε概率
INITIAL_EPSILON = 0.5 # 开始探索时候的选择动作的ε概率
...
class DQN:
	...
	def createNetwork(self):
		...
		# 输入层
		self.inputState = tf.placeholder("float", [None, 80, 80, 4])

		# 第一层隐藏层+池化层 tf.nn.relu激活函数  80*80*4 ->  20*20*32  80/4 = 20
		h_conv1 = tf.nn.relu(self.conv2d(self.inputState, W_conv1, 4) + b_conv1) #  80*80*4 ->  20*20*32  80/4 = 20
		...
		# 输出层
		# readout layer 动作的Q值
		self.QValue = tf.matmul(h_fc1, W_fc2) + b_fc2 # 512*2 -- 2
	...
	def trainNetwork(self):  # 图片输入 输出层 全连接层 tf训练
		...
		# 计算标签值
		y_batch = []
		QValue_batch = self.QValue.eval(feed_dict = {self.inputState : next_state_batch})
	...
	def getAction(self):
		# 根据ε 概率选择一个Action
		QValue = self.QValue.eval(feed_dict={self.inputState : [self.currentState]})[0]
