...
class DQN_DUELING:
	...
	def createNetwork(self):
		...
		# 512 - 2 优势函数
		W_fc2A = self.weight_variable([512, ACTIONS])
		b_fc2A = self.bias_variable([1, ACTIONS])
		# 512 - 1 价值函数
		W_fc2V = self.weight_variable([512, 1])
		b_fc2V = self.bias_variable([1, 1])
		...
		#价值函数 和 优势函数
		eval_V = tf.matmul(h_fc1, W_fc2V) + b_fc2V
		eval_A = tf.matmul(h_fc1, W_fc2A) + b_fc2A
		
		# 输出层
		# readout layer 动作的Q值
		QValue = eval_V + (eval_A - tf.reduce_mean(eval_A, axis=1, keep_dims=True))  # Q = V(s) + A(s,a)
		
		return stateInput,QValue,W_conv1,b_conv1,W_conv2,b_conv2,W_conv3,b_conv3,W_fc1,b_fc1,W_fc2A,b_fc2A,W_fc2V,b_fc2V
